...
function ProjectModal({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-950/75 p-3 backdrop-blur-sm sm:p-6" onClick={onClose} role="presentation">
      <div role="dialog" aria-modal="true" aria-labelledby="project-modal-title" className="animate-fade-up relative mx-auto my-3 w-full max-w-5xl overflow-hidden rounded-[1.5rem] bg-white shadow-2xl dark:bg-[#0d1711] sm:my-8 sm:rounded-[2.5rem]" onClick={(event) => event.stopPropagation()}>
        <button type="button" onClick={onClose} aria-label={t("projects.close")} className="absolute end-4 top-4 z-20 grid h-10 w-10 place-items-center rounded-xl bg-slate-950/60 text-white backdrop-blur transition hover:bg-slate-950 sm:end-5 sm:top-5">
        ...
        <div className="p-5 sm:p-8 lg:p-10">
          ...
          <div className="mt-7 grid gap-px overflow-hidden rounded-2xl border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-4 dark:border-white/10 dark:bg-white/10">
            {[
              ...
              <div key={key} className="bg-white p-4 dark:bg-[#111d16]"><div className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">{t(key)}</div><div className="mt-1.5 text-sm font-bold leading-6 text-slate-800 dark:text-slate-100">{value}</div></div>
            ))}
          </div>
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="scroll-mt-28 bg-white py-20 sm:py-28 dark:bg-[#09110d]">
      <Container>
        ...
        <div className="mt-10 flex max-w-full gap-2 overflow-x-auto pb-2 sm:flex-wrap" role="group" aria-label={t("projects.kicker")}>
          {categories.map((item) => (
            <button type="button" key={item} onClick={() => setCategory(item)} aria-pressed={category === item} className={`shrink-0 rounded-xl px-4 py-2.5 text-xs font-extrabold transition ${category === item ? "bg-leaf-700 text-white shadow-[0_12px_24px_-16px_rgba(21,128,61,0.9)] dark:bg-leaf-500" : "border border-slate-200 bg-white text-slate-600 hover:border-leaf-300 hover:bg-leaf-50 hover:text-leaf-800 dark:border-white/10 dark:bg-white/5 dark:text-slate-300"}`}>
        ...
        <div className="mt-9 grid gap-5 sm:grid-cols-2 lg:grid-cols-12">
          {visibleProjects.map((project, index) => {
            ...
            return (
              <article key={project.id} className={`group flex min-w-0 flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white transition duration-300 hover:-translate-y-1 hover:border-leaf-200 hover:shadow-[0_28px_70px_-44px_rgba(15,23,42,0.55)] dark:border-white/10 dark:bg-white/[0.035] ${cardSpan(index)}`}>
                ...
                <div className="flex flex-1 flex-col p-5 sm:p-6">
                  ...
                  <h3 className={`${featured ? "text-xl sm:text-2xl" : "text-lg"} mt-3 font-black leading-tight tracking-[-0.025em] text-slate-950 dark:text-white`}>{project.title[lang]}</h3>
                  ...
                  <button type="button" onClick={() => setOpenProject(project)} className="mt-5 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 text-xs font-extrabold text-white transition hover:bg-leaf-700 dark:bg-white dark:text-slate-950 dark:hover:bg-leaf-300">
